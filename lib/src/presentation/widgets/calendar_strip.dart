...
import 'package:my_gym_coach/src/utils/datetime/date_utils.dart' as du;
...
class CalendarStripState extends State<CalendarStrip>
  ...
  TextStyle monthLabelStyle = const TextStyle(
  ...
  CalendarStripState(
    ...
    lastDayOfMonth = du.DateUtils.getLastDayOfMonth(currentDate);
  ...
  String getMonthLabel() {
    DateTime? startingDayObj = rowStartingDate,
        endingDayObj = rowStartingDate!.add(const Duration(days: 6));
    String label = "";
    ...
    if (doesDateRangeExists) {
      ...
      if (mode != null) {
        _nextRowStartingDate = mode == "PREV"
            ? rowStartingDate!.subtract(const Duration(days: 7))
            : rowStartingDate!.add(const Duration(days: 7));
      } else {
        _nextRowStartingDate = rowStartingDate;
      }
      weekStartingDate = getDateOnly(_nextRowStartingDate!);
      weekEndingDate =
          getDateOnly(_nextRowStartingDate.add(const Duration(days: 6)));
